...
[English](iterative-step-runs-design.md) | 简体中文
...
5. 若要求修改，审核人提交 `decision=request_revision`，run 被标记为 `rejected`，并可触发新的 rerun

`request_revision` 不意味着覆盖原运行记录；系统会保留被拒绝的尝试，并创建新的 `StepRun`。
